...
$Logger = Initialize-ScriptLogger -LogDirectory $logDirectory -ScriptName $scriptName -Step $step -Config $Config
$Log = $Logger.Logger
...
Use-ValidDirectoriesRecursively -RootDirectory $processedDirectory

& $Log "INFO" "--- Starting File Name Sanitization ---"
Use-ValidFilesRecursively -RootDirectory $processedDirectory
