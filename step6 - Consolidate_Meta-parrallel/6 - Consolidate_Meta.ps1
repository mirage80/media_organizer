...
$start = 10000
$end = 10100
$subset = $filesToProcess[$start..$end]
    ...
    if ($result.success) {
        ...
        if (Test-Path -Path $hashLogPath) {
            try {
                ...
                Write-Warning "⚠️ Failed to load existing log. Will overwrite with fresh log."
                $existingLog = @()
            }
        }
        ...
        Write-Warning "❌ Failed: $($result.path)`n  → $($result.error)"
    }
}
