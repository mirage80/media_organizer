...
$processedDirectory = ConvertTo-StandardPath -Path $processedDirectory
$RecycleBinPath = Join-Path -Path $processedDirectory -ChildPath '$RECYCLE.BIN'
...
$Logger = Initialize-ScriptLogger -LogDirectory $logDirectory -ScriptName $scriptName -Step $phase
$Log = $Logger.Logger
...
if (Test-Path -Path $RecycleBinPath -PathType Container) {
    ...
    & $Log "INFO" "No '$RECYCLE.BIN' directory found in '$processedDirectory'. Nothing to do."
}
