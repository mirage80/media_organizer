...
def remove_files_not_available(grouping_json_path, video_info_json_path):
    """
    Removes entries for files that are no longer available on disk from both
    grouping_info.json and video_info.json.
    ...
    try:
        ...
        print(f"Error: File not found at {video_info_json_path}")
        return
    except json.JSONDecodeError:
        print(f"Error: Invalid JSON format in {video_info_json_path}")
        return

    if "videos" not in video_info_data:
        print("Error: 'videos' key not found in video_info JSON data.")
        return

    # Remove entries for files that no longer exist
    original_count = len(video_info_data["videos"])
    video_info_data["videos"] = [
        video for video in video_info_data["videos"] if os.path.exists(video["path"])
    ]
    updated_count = len(video_info_data["videos"])

    print(f"Removed {original_count - updated_count} entries from video_info.json.")

    # Save the cleaned video_info.json
    try:
        with open(video_info_json_path, 'w') as f:
            json.dump(video_info_data, f, indent=4)
    except OSError as e:
        print(f"Error updating video info JSON file: {e}")

def remove_duplicate_videos(json_file_path):
    """
    Removes duplicate video files within each group in the given JSON file, 
...
remove_files_not_available(GROUPING_INFO_FILE, VIDEO_INFO_FILE)
remove_duplicate_videos(GROUPING_INFO_FILE)
