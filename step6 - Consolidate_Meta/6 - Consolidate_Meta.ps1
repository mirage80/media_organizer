...
# Validate $unzipedDirectory
if (-not $unzipedDirectory) {
    Write-Warning "The variable 'unzipedDirectory' is not defined. Exiting..."
    exit
}
...
$currentItem = 0
$totalItems = $files.Count  # Extract count before passing

# Precompute values before entering the parallel script block
$errors = [System.Collections.Concurrent.ConcurrentBag[string]]::new()

# Process files
foreach ($file in $files) {
    $currentItem++
    Show-ProgressBar -Current $currentItem -Total $totalItems -Message $file
    try {
        # Process the file
        Consolidate_Json_Exif_Data -File $file
    } catch {
        # Store error message in a thread-safe collection
        $errors.Add("Error processing file: $($file.FullName). See previous logs for details.")
    }
} 

# Log errors at the end
if ($errors.Count -gt 0) {
    Write-Warning "The following errors occurred during processing:"
    $errors | ForEach-Object { Write-Warning $_ }
}

#remove original files
Remove_FilesWithOriginalExtension -root $([System.IO.DirectoryInfo]$DestDirectory.Directory)

#remove json files
Get-ChildItem -Path $unzipedDirectory -recurse -Filter "*.json" | Remove-Item
